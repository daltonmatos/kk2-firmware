	...
	// This code supports only DSMX 2048 (11 bit and all channels in a single frame, 16 bytes)
...
sat3:	ldz Channel1L			;set Channel Array to 1st location
	...
	lsr t				;shift the channel ID twice for 11 bit data
	lsr t
...
sat2:	andi yh, 0x07			;mask for MSB
...
gcv1:	ldzarray Channel1L, 2, r0	;register R0 (input parameter) holds the mapped channel ID
	...
	ld xh, z
	clr yh
	andi xh, 0x07			;limit value to 2047
	ret
