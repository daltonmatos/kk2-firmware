...
eei3:	call StoreEeVariable8
	...
	ldx eeStickScaleRoll		;write the remaining parameters (68 bytes)
	ldi Counter, 68
	rcall WriteEeArray
...
eei5:	.dw 50, 100, 25, 20		;default PI gains and limits for aileron, elevator and rudder
	...
	.dw 30, 30, 50, 90, 100		;default stick scaling settings							10 bytes

	.dw 60, 20, 0, 0, 10		;default self-level settings							10 bytes

	.dw 10, 0, 30, 0, 50		;default misc settings								10 bytes

	.db 0xFF, 0xFF, 0xFF, 0xFF	;default mode settings (plus padding)						6 bytes
	.db 0x00, 0x00

	.dw 0, 0, 0, 0, 0, 0		;default gimbal settings (plus padding)						14 bytes
	.db 0x00, 0x00

	.db 0, 3, 1, 3, 2, 0		;default aux switch settings (plus padding)					6 bytes

	.db 2, 3, 1, 4, 5, 6, 7, 8	;default RX channel order							8 bytes

	.db 0x00, 0x00, 0x00, 0x02	;eeSensorsCalibrated, eeMotorLayoutOk, eeUserAccepted and eeTuningRate		4 bytes

					;									TOTAL: 68 bytes

eesign:	.db 0x19, 0x03, 0x73, 0xD4	;EE signature (must be the last item)
