KK2.0 v1.6++ Satellite R4 firmware by RC911


Features (Revision 4)
=====================
This custom firmware is based on the original KK2.0 1V6 firmware by Rolf Bakke. It has the following features:
- This version is for Spektrum Satellite units only. It supports DSM2 and DSMX (7 channels). The DSM2 RX code was borrowed from Steveis' Sat firmware, but it has been heavily modified by me.
- Spektrum receiver binding feature was made by David Thompson and fixed by Steveis. Thanks :)
...
Connections
===========
To get data from your Satellite unit you will need a special converter cable connected to the throttle input connector on your KK2 board. The other input pins should remain disconnected.
The 'SAFE' screen will show a "No Satellite data" message until a valid data stream has been detected (unless there are other, more important messages to display).


Satellite binding
=================
Binding will be necessary if you see a "Sat protocol error" message on the 'SAFE' screen. Hold down button 2 and 3 on your KK2 board while connecting the flight battery. This should configure the Satellite unit to use the correct protocol.


DSM2/DSMX
=========
To use DSM2 mode the kk2dsm2.hex file must be flashed. For DSMX mode the kk2dsmx.hex file must be flashed. All settings will be preserved when switching to the other file.
...
Observe:
- To access these features you will have to map one of your real AUX input channels to AUX4 by using the Channel Mapping feature.
